...
//자주찾는질문
const group = document.querySelectorAll('.category > li > a');
const group_ul = document.querySelectorAll('.faq__list');

for (let k = 0; k < group.length; k++) {
    if(k!=0)
        group_ul[k].style.display = 'none';
    group[k].addEventListener('click', e => {
        e.preventDefault();

        for (let j = 0; j < group_ul.length; j++) {
            if (k == j) {
                group_ul[j].style.display = 'block';
                continue;
            }
            group_ul[j].style.display = 'none';
        }
    });
}
